...
```json
{
    "sort-keys": ["error", "asc", {"insensitive": false, "natural": false}]
}
```
...
Examples of **incorrect** code for the `{insensitive: true}` option:

```js
/*eslint sort-keys: ["error", "asc", {insensitive: true}]*/
/*eslint-env es6*/
...
Examples of **correct** code for the `{insensitive: true}` option:
